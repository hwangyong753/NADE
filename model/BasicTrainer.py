...
class Trainer(object):
    ...
    def val_epoch(self, epoch, val_dataloader):
        ...
        y_true = self.scaler.inverse_transform(torch.cat(y_true, dim=0))[:,:,:,:1]

        if self.args.real_value:
            y_pred = torch.cat(y_pred, dim=0)
        else:
            y_pred = self.scaler.inverse_transform(torch.cat(y_pred, dim=0))[:,:,:,:1]

        val_loss = self.loss(y_pred.cuda(), y_true)
    ...
    def test_epoch(self, epoch, test_dataloader):
        ...
        test_loss = self.loss(y_pred.cuda(), y_true)
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            if not self.args.real_value:
                output = self.scaler.inverse_transform(output)[:,:,:,:1]
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            # if epoch % 1000 == 0:
            #     current_model = copy.deepcopy(self.model.state_dict())
            #     self.model.load_state_dict(best_model)
            #     self.test(self.model, self.args, self.test_loader, self.scaler, self.logger)
            #     self.model.load_state_dict(current_model)


        training_time = time.time() - start_time
        self.logger.info("Total training time: {:.4f}min, best loss: {:.6f}, best test loss: {:.6f}".format((training_time / 60), best_loss, best_test_loss))
    ...
    def test(model, args, data_loader, scaler, logger, path=None):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(data_loader):
                ...
                y_true.append(label)
                y_pred.append(output)
        y_true = scaler.inverse_transform(torch.cat(y_true, dim=0))[:,:,:,:1]
        if args.real_value:
            y_pred = torch.cat(y_pred, dim=0)
        else:
            y_pred = scaler.inverse_transform(torch.cat(y_pred, dim=0))[:,:,:,:1]
        np.save('./{}_true.npy'.format(args.dataset), y_true.cpu().numpy())
        np.save('./{}_pred.npy'.format(args.dataset), y_pred.cpu().numpy())
        for t in range(y_true.shape[1]):
            mse, mae, rmse, mape, _, _ = All_Metrics(y_pred[:, t, ...], y_true[:, t, ...],
                                                args.mae_thresh, args.mape_thresh)
            logger.info("Horizon {:04d}, MSE: {:.4f}, MAE: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}%".format(
                t + 1, mse, mae, rmse, mape*100))
        mse, mae, rmse, mape, _, _ = All_Metrics(y_pred, y_true, args.mae_thresh, args.mape_thresh)
        logger.info("Average Horizon, MSE: {:.4f}, MAE: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}%".format(
