...
def get_dataloader(args, normalizer = 'std', tod=False, dow=False, weather=False, single=True):
    ...
    if args.time_dependence :
        t_idx = np.expand_dims(np.arange(data.shape[0]), axis=(1,2))
        if args.time_divided:
            t_idx = t_idx % 24 / 24
        t_idx = np.repeat(t_idx, data.shape[1], axis=1)
        data = np.concatenate((data,t_idx), axis=-1)
